...
export function ModelSelector({ models }: ModelSelectorProps) {
  ...
  useEffect(() => {
    ...
    if (savedModel) {
      try {
        console.log('Raw cookie value:', savedModel)
        // Try decoding multiple times until we get valid JSON
        let decodedModel = savedModel
        while (decodedModel.includes('%')) {
          try {
            decodedModel = decodeURIComponent(decodedModel)
            console.log('Decoded:', decodedModel)
          } catch (e) {
            break
          }
        }
        const model = JSON.parse(decodedModel) as Model
